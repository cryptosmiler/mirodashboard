<template lang="html">
  <div class="">
    ...
    <b-container fluid v-bind:class="{sideMenuOpen: sideMenuIsOpen, sideMenuClosed: sideMenuIsClosed}" class="account-container">
      <b-row no-gutters>
        <b-col md="7" lg="5" xl="4" class="event-side-menu" v-if="profile.role!==1">
          <gallery-side-menu-for-overlay/>
        </b-col>
...
import GallerySideMenuForOverlay from '../components/EditEvent/GallerySideMenuForOverlay'
import ImagePreview from '../components/EditEvent/ImagePreview.vue'
import { mapActions, mapGetters } from 'vuex'

export default {
  name: 'event-gallery-overlay',
  data () {
    return {
      show: false,
  ...
  created () {
    this.$store.dispatch('changeCurrentTab', 'GallerySettings')
    this.$store.dispatch("Prices/loadPrices")

    EventBus.$on('edit-gallery-url-modal-state', (payload) => {
     this.$bvModal.show('edit-gallery-url')
    })
