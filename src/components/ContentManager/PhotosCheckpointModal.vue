<template>
    <div id="photoCheckpointModal" class="modal photoCheckpointModal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-body">
                    <b-container fluid>
                        <b-row class="modal-header">
                            <b-col>Move these images to</b-col>
                        </b-row>
                        <b-row >
                            <b-col cols="12">
                                <div class="dropdown" v-if="toCheckpoint">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <a v-for="(item, index) in getCheckpoints" :key="index" class="dropdown-item" @click="select(item)">{{item.checkpoint_label}}</a>
                                    </div>
                                </div>
                            </b-col>
                            <b-col cols="12">
                                <button class="confirm-btn" :disabled="checkSameCheckpoint" @click="confirm()">Move</button>
                            </b-col>
                        </b-row>
                    </b-container>
                </div>
...
export default {
  ...
  computed: {
    ...
    checkSameCheckpoint() {
        return this.getSelectedImagesObjects.length > 0 && 
            this.getCheckpoints && this.originalCheckpoint && this.toCheckpoint &&
            this.originalCheckpoint.checkpoint_id === this.toCheckpoint.checkpoint_id
    }
  },
  methods: {
      setUp() {
        ...
        if(this.toCheckpoint) {
            $('#photoCheckpointModal').modal('show')
        } else {
            console.log("Cannot show model")
        }
      },
      confirm() {
        if(!this.checkSameCheckpoint) {
            // console.log('to:', this.toCheckpoint.checkpoint_id)
            this.$store.dispatch('Events/setImagesCheckpoint', this.toCheckpoint.checkpoint_id)
            $('#photoCheckpointModal').modal('hide')
        }
      },
      select(item) {
  ...
  mounted() {
      EventBus.$on('open-photos-checkpoint-modal', ()=> {
        this.setUp();
      })
  }
};
