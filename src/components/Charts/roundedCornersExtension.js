...
Chart.elements.Rectangle.prototype.draw = function() {
  ...
  if (!vm.horizontal) {
    ...
    // horizontal bar
    const { scales } = this._chart.options
    let reverse = false
    if (scales && scales.xAxes
      && scales.xAxes.length && scales.xAxes[0].ticks
      && scales.xAxes[0].ticks.reverse)
    {
        reverse = scales.xAxes[0].ticks.reverse
    }
    left = reverse ? vm.x : vm.base;
    right = reverse ? vm.base : vm.x;
